...
public class RedisInfo {
    ...
    static {
        map.put("tcp_port", "10|当前运行端口");
        map.put("redis_version", "11|Redis版本");
        map.put("process_id", "12|Redis PID");

        map.put("os", "20|Redis的宿主操作系统");
        map.put("arch_bits", "21|架构（32 或 64 位）");
        map.put("uptime_in_days", "23|Redis启动天数");
        map.put("connected_clients", "24|已连接客户端数量");

        map.put("used_memory_peak_human", "30|Redis内存消耗峰值");
        map.put("used_memory_human", "31|Redis申请的内存总量");
        map.put("used_memory_rss_human", "32|系统分配的内存总量");
        map.put("total_system_memory_human", "33|系统总内存");

        map.put("redis_mode", "41|运行模式");
        map.put("maxmemory_human", "42|最大内存限制");
        map.put("total_commands_processed", "43|已处理的命令数");
        map.put("instantaneous_ops_per_sec", "44|Redis内部较实时的每秒执行命令数");

        map.put("keyspace_hits", "51|命中次数");
        map.put("keyspace_misses", "52|没命中次数");

        map.put("multiplexing_api", "61|当前事件处理机制");
        map.put("aof_enabled", "62|是否开启aof");
        map.put("total_connections_received", "63|新创建连接个数");
        map.put("rejected_connections", "64|拒绝的连接个数");
...
//		map.put("db0", "db0的key的数量,以及带有生存期的key的数,平均存活时间");
    ...
    public void setKey(String key) {
        this.key = key;
        String infomation = map.get(this.key);
        if (!StringUtils.isEmpty(infomation)) {
            this.description = infomation.split("\\|")[1];
            this.sort = Integer.parseInt(infomation.split("\\|")[0]);
        }
    }
    ...
    public RedisInfo setSort(Integer sort) {
        this.sort = sort;
        return this;
    }
